...
import { removeFromDbCart, updateDbCartQuantity } from './actions'
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-3 flex items-center gap-4">
                       <div className="flex items-center bg-gray-100 rounded-xl p-1 px-3 gap-4">
                          <form action={async () => { 'use server'; await updateDbCartQuantity(item.id, item.quantity - 1); }}>
                       ...
                       <form action={async () => { 'use server'; await removeFromDbCart(item.id); }}>
                         <button type="submit" className="text-gray-400 hover:text-red-600"><Trash2 size={18} /></button>
                       </form>
                    </div>
                  </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-xl border h-fit sticky top-24">
              ...
              <div className="space-y-4 mb-8">
                <div className="flex justify-between font-medium text-gray-500"><span>পণ্যের দাম</span><span>৳{subtotal}</span></div>
                <div className="flex justify-between font-medium text-gray-500"><span>ডেলিভারি</span><span className="text-green-600">ফ্রি</span></div>
              ...
              <Link href="/checkout" className="w-full bg-red-600 text-white py-4 rounded-2xl font-black text-lg flex items-center justify-center gap-2 shadow-lg shadow-red-100 transition-all active:scale-95">
                অর্ডার সম্পন্ন করুন <ArrowRight size={20} />
              </Link>
            </div>
          </div>
        )}
